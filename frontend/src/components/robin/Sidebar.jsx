import { X, Home, MapPin, Truck } from "lucide-react";
import { motion } from "framer-motion";

export function Sidebar({ open, onClose, setActivePage }) {
  const menuItems = [
    { name: "Dashboard", icon: <Home size={18} />, action: "dashboard" },
    { name: "Select Route", icon: <MapPin size={18} />, action: "selectRoute" },
    { name: "Active Delivery", icon: <Truck size={18} />, action: "activeDelivery" },
  ];
